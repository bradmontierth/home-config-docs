...
# Trains both wake word candidates sequentially; logs to /work/train.log.
set -euo pipefail
exec > >(tee -a /work/train.log) 2>&1
...
livekit-wakeword setup --config configs/okay_computer.yaml

echo "=== $(date) candidate A: okay_computer ==="
livekit-wakeword run configs/okay_computer.yaml

echo "=== $(date) candidate B: hey_computer ==="
livekit-wakeword run configs/hey_computer.yaml
