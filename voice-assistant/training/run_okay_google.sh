...
# Trains okay_google only; logs to /work/train_okay_google.log.
# Identical prep to run_training.sh (2026-07-06 run) incl. the unified-memory
# OOM cap — see that script for the history. Do not launch uncapped.
set -euo pipefail
exec > >(tee -a /work/train_okay_google.log) 2>&1
...
livekit-wakeword setup --config configs/okay_google.yaml

echo "=== $(date) okay_google ==="
livekit-wakeword run configs/okay_google.yaml

echo "=== $(date) ALL DONE ==="
ls -la /work/output/okay_google/
