...
Slideshow video AUDIO plays on the kitchen voice satellite (kitchen-speaker
:8781), not the display — the display has no speaker and the video element
stays muted by design. The slideshow service extracts WAVs at transcode time
and relays them via the satellite's `/media/play`/`/media/stop` (music ducks
automatically; wake words and alarms kill the clip audio). Expect a small
A/V offset (two devices, no shared clock) — acceptable per Brad until the
display-pi power supply is fixed; a local USB speaker would give perfect
sync (then unmute in viewer.html and add
